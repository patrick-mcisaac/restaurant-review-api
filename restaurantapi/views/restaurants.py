...
from restaurantapi.models import Restaurant, RestaurantLocation
...
class Restaurants(ViewSet):
    def list(self, request):
        ...
        location = request.query_params.get("locations", None)
        restaurants = Restaurant.objects.all()
        if search is not None and search != " ":
            restaurants = restaurants.filter(name__contains=search)
        if location is not None and int(location) > 0:
            restaurants = restaurants.filter(locations__city__id=location)
        ser = RestaurantSerializer(
            restaurants,
            many=True,
            context={"restaurants": restaurants, "user": request.auth.user},
        )
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            ser = RestaurantSerializer(
                restaurant,
                many=False,
                context={"restaurant": restaurant, "user": request.auth.user},
            )
            return Response(ser.data, status=status.HTTP_200_OK)
